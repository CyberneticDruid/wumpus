...
void print_instructions() {
    ...
    /* 430 PRINT "     GO THERE, A BAT GRABS YOU AND TAKES YOU TO SOME
     * OTHER"   */
    ...
    for (k = 0; k < j9; k++) {
        ...
        for (k1 = 0; k1 < 3; k1++) {
            ...
            if (cave[scratchloc][k1] == path[k]) {
                /*
                 * This is the only bit of the translation I'm
                 * not sure about.  It requires the trajectory
                 * of the arrow to be a path.  Without it, all
                 * rooms on the trajectory would be required by
                 * the above to be adjacent to the player,
                 * making for a trivial game --- just move to
                 * where you smell a wumpus and shoot into all
                 * adjacent passages! However, I can't find an
                 * equivalent in the BASIC.
                 */
                scratchloc = path[k];
...
goodmove:
    ...
    if (scratchloc == loc[WUMPUS]) {
        ...
        /* Fall through since Wumpus could have been in a pit or bat
         * room */
    }
...
newgame:
    ...
    (void)printf("Wumpus is at %d, pits at %d & %d, bats at %d & %d\n",
                 loc[WUMPUS] + 1, loc[PIT1] + 1, loc[PIT2] + 1,
